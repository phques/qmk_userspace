...
#    ifdef LAYOUT_ADAPTIVE_INC
    ...
    for (int i = 0; pgm_read_ptr(&adaptive_3keys[i].output) != NULL; i++) {
        if (keycode == pgm_read_word(&adaptive_3keys[i].key3) && prior_keycode == pgm_read_word(&adaptive_3keys[i].key2) && preprior_keycode == pgm_read_word(&adaptive_3keys[i].key1)) {
    ...
    for (int i = 0; return_state && pgm_read_ptr(&adaptive_2keys[i].output) != NULL; i++) {
        if (keycode == pgm_read_word(&adaptive_2keys[i].key2) && prior_keycode == pgm_read_word(&adaptive_2keys[i].key1)) {
            // We have a match! Output the string.
            SEND_STRING("\b");
